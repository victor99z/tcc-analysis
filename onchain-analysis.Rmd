...
# leitura de todos os datasets para onchain
...
# Plot for system_memory_used
plot_memory1 <- ggplot(df_metrics_recv_join[df_metrics_recv_join$name == "system_memory_used", ], aes(x = scenario, y = avg_value)) +
  geom_line(aes(group = name)) +
  geom_point(size = 2) +
  labs(y = "Memória (MB)", x = "") +
  theme_bw()
...
# Plot for system_cpu_sysload
plot_cpu_sysload1 <- ggplot(df_metrics_recv_join[df_metrics_recv_join$name == "system_cpu_sysload", ], aes(x = scenario, y = avg_value)) +
  geom_line(aes(group = name)) +
  geom_point(size = 2) +
  labs(y = "CPU (%)", x = "") +
  theme_bw()
...
plot_tps1 <- ggplot(df_avg_tps_recv_onchain, aes(x = scenario, y = avg_value)) +
  geom_line(aes(group = type)) +
  geom_point(size = 2) +
  labs(y = "TPS", x = "") +
  theme_bw()
