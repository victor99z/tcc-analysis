...
data_grouped <- data.frame(
  Latency = c(bs4n, bs8n, bs12n, bs16n),
  Nodes = factor(rep(c(4, 8, 12, 16), times = c(length(bs4n), length(bs8n), length(bs12n), length(bs16n))))
)
...
tmp <- data_grouped %>%
  group_by(Nodes, Latency) %>%
  filter(Nodes == "4")
  
mean_latency <- mean(tmp$Latency); round(mean_latency,1)
cv <- sd(tmp$Latency) / mean_latency; round(cv,2)
