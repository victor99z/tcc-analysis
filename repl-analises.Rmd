...
bench_filtered$nodes <- factor(bench_filtered$nodes, levels = c(4, 8, 12, 16), labels = c("4 nós", "8 nós", "12 nós", "16 nós"))
...
# ggsave('./figures/repl-boxplot-new.png', qt)
...
filter1 <- unnested_filter %>%
  group_by(nodes, label) %>%
  summarise(
    avg_tps = mean(transactions, na.rm = TRUE),
    sd_tps = sd(transactions, na.rm = TRUE),
    .groups = 'drop'
  )
...
ppp <- ggplot(filter1, aes(x = nodes, y = avg_tps, fill = label)) +
  ...
  # geom_errorbar( aes(x=nodes, ymin=avg_tps-sd, ymax=avg_tps+sd), width=0.2, colour="black", alpha=0.9, size=0.8) + 
  geom_errorbar(aes(ymin=avg_tps-sd_tps, ymax=avg_tps+sd_tps), width=.2,position=position_dodge(.9)) +
...
# ggsave('./figures/repl-barra-tps-revisado.png',ppp)
```
